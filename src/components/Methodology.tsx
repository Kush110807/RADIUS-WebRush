  ...
  return (
    <main className="methodology-view">
      <div className="methodology-wrap">
        <button className="text-button" onClick={onBack}>← Back to story</button>
        ...
        <section>
          ...
          <p>
            The raw EMA table contains {v.generalStressResponses} non-missing stress responses; {v.deployedStressDays} align to the deployed sensing-day archive. RADIUS keeps that distinction rather than manufacturing a value for a day that is not present in the daily sensing series.
          </p>
          <div className="method-stats">
            <span><b>{v.locationCoverageDays.toLocaleString()}</b> days with location coverage</span>
            <span><b>{v.deployedStressDays}</b> deployed self-reported stress days</span>
            <span><b>{v.deployedCovidDays}</b> usable COVID-response days</span>
            <span><b>{v.deployedBackgroundApplicationObservations.toLocaleString()}</b> app observations aligned to sensing days</span>
            <span><b>{v.rawCallRecords.toLocaleString()}</b> raw call records verified</span>
            <span><b>{v.rawSmsRecords.toLocaleString()}</b> raw SMS records verified</span>
          </div>
        </section>
        ...
        <section>
          <h2>Source</h2>
          <p>
            College Experience Study dataset by Subigya Nepal and collaborators; published study DOI: 10.1145/3643501. Dataset licence: CC BY-NC-SA 4.0.
          </p>
        </section>
      </div>
    </main>
